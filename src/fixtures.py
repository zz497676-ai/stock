"""--mock 模式假数据:列名与 akshare 1.18 各接口真实返回一致,用于离线跑通全流程。"""

from __future__ import annotations

from datetime import date, timedelta

import pandas as pd


def _trading_days_back(d: date, n: int) -> list[date]:
    out, cur = [], d
    while len(out) < n:
        if cur.weekday() < 5:
            out.append(cur)
        cur -= timedelta(days=1)
    return list(reversed(out))


def build_mock_data(d: date) -> dict[str, pd.DataFrame]:
    days = _trading_days_back(d, 30)
    ds = [x.isoformat() for x in days]

    return {
        "stock_lhb_detail_em": pd.DataFrame(
            {
                "代码": ["600519", "300750", "002131"],
                "名称": ["贵州茅台", "宁德时代", "利欧股份"],
                "上榜日": [d.isoformat()] * 3,
                "收盘价": [1500.0, 210.5, 3.21],
                "涨跌幅": [3.2, -5.1, 10.02],
                "龙虎榜净买额": [2.1e8, -3.5e8, 1.2e8],
                "龙虎榜买入额": [5e8, 2e8, 3e8],
                "龙虎榜卖出额": [2.9e8, 5.5e8, 1.8e8],
                "龙虎榜成交额": [7.9e8, 7.5e8, 4.8e8],
                "上榜原因": ["日涨幅偏离值达7%", "日跌幅偏离值达7%", "连续三个交易日内涨幅偏离值累计20%"],
            }
        ),
        "stock_lhb_hyyyb_em": pd.DataFrame(
            {
                "营业部名称": [
                    "华鑫证券有限责任公司上海分公司",
                    "东方财富证券股份有限公司拉萨团结路第二证券营业部",
                    "中信证券股份有限公司上海溧阳路证券营业部",
                ],
                "上榜日": [d.isoformat()] * 3,
                "买入个股数": [5, 12, 3],
                "卖出个股数": [2, 10, 4],
                "买入总金额": [4.2e8, 2.8e8, 3.1e8],
                "卖出总金额": [1.1e8, 2.5e8, 3.8e8],
                "总买卖净额": [3.1e8, 0.3e8, -0.7e8],
                "买入股票": ["利欧股份 贵州茅台", "N易联 利欧股份", "宁德时代"],
            }
        ),
        "stock_ggcg_em": pd.DataFrame(
            {
                "代码": ["000001", "600036", "002415"],
                "名称": ["平安银行", "招商银行", "海康威视"],
                "股东名称": ["平安集团", "招银国际", "龚虹嘉"],
                "持股变动信息-增减": ["增持", "增持", "减持"],
                "持股变动信息-变动数量": [1e7, 5e6, -8e6],
                "持股变动信息-占总股本比例": [0.05, 0.02, 0.09],
                "公告日": [d.isoformat(), d.isoformat(), d.isoformat()],
            }
        ),
        "stock_repurchase_em": pd.DataFrame(
            {
                "股票代码": ["600519", "000333"],
                "股票简称": ["贵州茅台", "美的集团"],
                "实施进度": ["回购实施", "回购完成"],
                "已回购金额": [3e8, 12e8],
                "最新公告日期": [d.isoformat(), d.isoformat()],
            }
        ),
        "stock_dzjy_sctj": pd.DataFrame(
            {
                "交易日期": ds[-5:],
                "大宗交易成交总额": [80e8, 95e8, 70e8, 110e8, 88e8],
                "溢价成交总额占比": [12.0, 8.5, 15.2, 22.1, 18.0],
                "折价成交总额占比": [70.0, 75.0, 68.0, 60.0, 65.0],
            }
        ),
        "stock_dzjy_mrmx": pd.DataFrame(
            {
                "证券代码": ["600519", "000333"],
                "证券简称": ["贵州茅台", "美的集团"],
                "成交价": [1480.0, 65.0],
                "折溢率": [-3.2, 0.0],
                "成交量": [10000, 50000],
                "成交额": [1.48e7, 3.25e6],
                "买方营业部": ["机构专用", "中信证券北京分公司"],
                "卖方营业部": ["机构专用", "机构专用"],
            }
        ),
        "stock_margin_sse": pd.DataFrame(
            {
                "信用交易日期": [x.strftime("%Y%m%d") for x in days],
                "融资余额": [9000e8 + i * 10e8 for i in range(len(days))],
                "融资买入额": [800e8] * len(days),
            }
        ),
        # 深交所口径:单位亿元
        "stock_margin_szse": pd.DataFrame(
            {"融资余额": [8500.0], "融资买入额": [700.0], "融资融券余额": [8700.0]}
        ),
        "stock_account_statistics_em": pd.DataFrame(
            {
                "数据日期": ["2026-05", "2026-06"],
                "新增投资者-数量": [120.5, 135.2],
                "新增投资者-环比": ["+5.2%", "+12.2%"],
            }
        ),
        "fund_etf_spot_em": pd.DataFrame(
            {
                "代码": ["510300", "510050", "588000"],
                "名称": ["沪深300ETF", "上证50ETF", "科创50ETF"],
                "最新价": [3.85, 2.71, 1.02],
                "涨跌幅": [-1.4, -1.1, -2.0],
                "成交额": [45e8, 20e8, 15e8],
                "最新份额": [1200e8, 900e8, 800e8],
            }
        ),
        "fund_new_found_em": pd.DataFrame(
            {
                "基金代码": ["021234", "021235"],
                "基金简称": ["某某沪深300指数A", "某某灵活配置混合"],
                "基金类型": ["指数型-股票", "混合型-灵活"],
                "募集份额": [12.5, 8.0],
                "成立日期": [(d - timedelta(days=2)).isoformat(), (d - timedelta(days=5)).isoformat()],
            }
        ),
        # 中证官网指数行情(mock 下 000300/932000 共用一张表,足够跑通逻辑)
        "stock_zh_index_hist_csindex": pd.DataFrame(
            {
                "日期": ds[-5:],
                "涨跌幅": [0.3, -0.5, 0.8, 0.2, -1.2],
                "成交金额": [2000.0, 2100.0, 1900.0, 2200.0, 2400.0],  # 亿元
            }
        ),
        # 交易所官方成交概况
        "stock_sse_deal_daily": pd.DataFrame(
            {"单日情况": ["挂牌数", "成交金额", "成交量"], "股票": [2300, 5100.0, 800.0]}  # 亿元
        ),
        "stock_szse_summary": pd.DataFrame(
            {"证券类别": ["股票", "基金"], "成交金额": [6300e8, 500e8], "数量": [2900, 1000]}  # 元
        ),
        "stock_individual_fund_flow_rank": pd.DataFrame(
            {
                "代码": ["600519", "300750"],
                "名称": ["贵州茅台", "宁德时代"],
                "今日小单净流入-净额": [45e8, 40e8],
                "今日主力净流入-净额": [-50e8, -42e8],
            }
        ),
        # 个股杠杆监测:沪深两市融资融券明细(T+1 披露,单位:元)
        "stock_margin_detail_sse": pd.DataFrame(
            {
                "信用交易日期": [d.strftime("%Y%m%d")] * 2,
                "标的证券代码": ["600519", "601628"],
                "标的证券简称": ["贵州茅台", "中国人寿"],
                "融资余额": [50e8, 30e8],
                "融资买入额": [2e8, 1.5e8],
                "融资偿还额": [1.8e8, 1.2e8],
                "融券余量": [1000.0, 500.0],
                "融券卖出量": [200.0, 100.0],
                "融券偿还量": [150.0, 80.0],
            }
        ),
        "stock_margin_detail_szse": pd.DataFrame(
            {
                "证券代码": ["002131", "300750"],
                "证券简称": ["利欧股份", "宁德时代"],
                "融资买入额": [3.5e8, 6e8],
                "融资余额": [8e8, 150e8],
                "融券卖出量": [50.0, 300.0],
                "融券余量": [300.0, 1500.0],
                "融券余额": [0.3e8, 4e8],
                "融资融券余额": [8.3e8, 154e8],
            }
        ),
        # 个股杠杆监测:全市场实时行情快照(用于取最新价/成交额/流通市值/振幅)
        "stock_zh_a_spot_em": pd.DataFrame(
            {
                "代码": ["600519", "601628", "002131", "300750"],
                "名称": ["贵州茅台", "中国人寿", "利欧股份", "宁德时代"],
                "最新价": [1500.0, 36.0, 3.21, 210.5],
                "成交额": [80e8, 40e8, 12e8, 210e8],
                "流通市值": [20000e8, 8000e8, 60e8, 9000e8],
                "涨跌幅": [3.2, 1.1, 10.02, -5.1],
                "振幅": [4.0, 2.5, 12.5, 6.0],
            }
        ),
        # 个股杠杆监测:备用行情源(新浪,em 不可用时的 fallback;不含流通市值)
        "stock_zh_a_spot": pd.DataFrame(
            {
                "代码": ["sh600519", "sh601628", "sz002131", "sz300750"],
                "名称": ["贵州茅台", "中国人寿", "利欧股份", "宁德时代"],
                "最新价": [1500.0, 36.0, 3.21, 210.5],
                "涨跌额": [46.5, 0.4, 0.29, -11.3],
                "涨跌幅": [3.2, 1.1, 10.02, -5.1],
                "买入": [1500.0, 36.0, 3.21, 210.5],
                "卖出": [1500.1, 36.01, 3.22, 210.6],
                "昨收": [1453.5, 35.6, 2.92, 221.8],
                "今开": [1460.0, 35.7, 2.95, 220.0],
                "最高": [1510.0, 36.2, 3.29, 215.0],
                "最低": [1450.0, 35.5, 2.90, 208.0],
                "成交量": [500000.0, 8000000.0, 40000000.0, 9000000.0],
                "成交额": [80e8, 40e8, 12e8, 210e8],
                "时间戳": [d.isoformat()] * 4,
            }
        ),
        # 两融历史切片(scripts/build_margin_db.py 回补收盘价用)。
        # mock 下 safe_fetch 不看入参,所有个股拿到同一份序列,够验证切片/打标签流程了。
        "stock_zh_a_hist": pd.DataFrame(
            {
                "日期": [(d - timedelta(days=n)).isoformat() for n in range(29, -1, -1)],
                "收盘": [100.0 + n * 0.5 for n in range(30)],
                "成交额": [1e8 + n * 1e6 for n in range(30)],
            }
        ),
        # ---- 市场温度评分 ----
        # 涨停股池:字段名已在 GitHub Actions 实测确认(见 specs/market-temperature.md §5),
        # 不是凭 akshare 文档记忆瞎猜的。含一只 *ST 用于测试 ST 剔除逻辑。
        "stock_zt_pool_em": pd.DataFrame(
            {
                "序号": [1, 2, 3],
                "代码": ["002432", "002677", "600001"],
                "名称": ["九安医疗", "浙江美大", "*ST某股"],
                "涨跌幅": [10.0, 9.96, 5.0],
                "最新价": [79.2, 7.95, 3.3],
                "成交额": [3.59e8, 0.55e8, 0.2e8],
                "流通市值": [368e8, 43.8e8, 10e8],
                "总市值": [369e8, 51.4e8, 12e8],
                "换手率": [9.8, 1.2, 3.0],
                "封板资金": [4.1e8, 0.9e8, 0.1e8],
                "首次封板时间": ["092500", "092500", "100000"],
                "最后封板时间": ["135900", "092500", "100000"],
                "炸板次数": [2, 0, 0],
                "涨停统计": ["4/3", "6/4", "1/1"],
                "连板数": [1, 3, 1],
                "所属行业": ["医疗器械", "厨卫电器", "其他"],
            }
        ),
        "stock_zt_pool_dtgc_em": pd.DataFrame(
            {
                "序号": [1],
                "代码": ["603391"],
                "名称": ["力聚热能"],
                "涨跌幅": [-10.0],
                "最新价": [52.91],
                "成交额": [1.1e8],
                "流通市值": [12.0e8],
                "总市值": [48.1e8],
                "动态市盈率": [321.5],
                "换手率": [8.6],
                "封单资金": [0.05e8],
                "最后封板时间": ["150000"],
                "板上成交额": [0.08e8],
                "连续跌停": [1],
                "开板次数": [11],
                "所属行业": ["专用设备"],
            }
        ),
        "stock_zt_pool_zbgc_em": pd.DataFrame(
            {
                "序号": [1],
                "代码": ["603779"],
                "名称": ["威龙股份"],
                "涨跌幅": [-0.63],
                "最新价": [7.89],
                "涨停价": [8.73],
                "成交额": [5.02e8],
                "流通市值": [26.2e8],
                "总市值": [26.2e8],
                "换手率": [18.2],
                "涨速": [0.0],
                "首次封板时间": ["093010"],
                "炸板次数": [1],
                "涨停统计": ["0/0"],
                "振幅": [10.8],
                "所属行业": ["非白酒"],
            }
        ),
        "stock_zt_pool_previous_em": pd.DataFrame(
            {
                "序号": [1, 2],
                "代码": ["000014", "000504"],
                "名称": ["沙河股份", "南华生物"],
                "涨跌幅": [-6.3, 2.1],
                "最新价": [9.37, 8.55],
                "涨停价": [11.0, 10.3],
                "成交额": [2.05e8, 2.55e8],
                "流通市值": [22.7e8, 28.1e8],
                "总市值": [22.7e8, 28.2e8],
                "换手率": [8.7, 8.8],
                "涨速": [0.0, 0.0],
                "振幅": [7.6, 7.4],
                "昨日封板时间": ["105833", "100515"],
                "昨日连板数": [1, 2],
                "涨停统计": ["2/1", "3/2"],
                "所属行业": ["房地产开", "医疗服务"],
            }
        ),
        # 乐咕赚钱效应:item 是"上涨"这类短标签,不带"家数"后缀(实测确认,见 spec §5)
        "stock_market_activity_legu": pd.DataFrame(
            {
                "item": ["上涨", "下跌", "平盘", "涨停", "跌停", "停牌"],
                "value": [3000.0, 1500.0, 100.0, 45.0, 3.0, 20.0],
            }
        ),
        # 深证成指/创业板指备源(新浪,em 不可用时才会用到东财备源,mock 下走不到那条分支):
        # 30 个交易日收盘价,用于测试收盘价环比自算涨跌幅
        "stock_zh_index_daily": pd.DataFrame(
            {
                "date": ds,
                "open": [10.0 + i * 0.01 for i in range(len(ds))],
                "high": [10.1 + i * 0.01 for i in range(len(ds))],
                "low": [9.9 + i * 0.01 for i in range(len(ds))],
                "close": [10.0 + i * 0.01 for i in range(len(ds))],
                "volume": [1e8] * len(ds),
            }
        ),
        "stock_notice_report": pd.DataFrame(
            {
                "代码": ["601628", "600000"],
                "名称": ["中国人寿", "浦发银行"],
                "公告标题": [
                    "中国人寿保险股份有限公司关于举牌某公司股票的公告",
                    "浦发银行关于股东增持计划的公告",
                ],
                "公告类型": ["持股变动", "持股变动"],
                "公告日期": [d.isoformat(), d.isoformat()],
                "网址": ["https://example.com/1", "https://example.com/2"],
            }
        ),
    }
