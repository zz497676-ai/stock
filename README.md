...
- 六宫格主视图默认**5分钟K**,只看最近一个交易日 9:15-15:00 这一段的分时变化,不会把好几天的K线堆在一张图里
  (拖动/滚轮可以往前翻更早的历史,点"⟲"缩回当天视野)。现价/涨跌幅每 5 秒(可调 3~60 秒,也可暂停)随实时报价刷新,
  同步推动最新一根K线:时间还没跨入下一根5分钟就原地更新收盘价/最高/最低,已经跨入新的5分钟就先补一根K线
  (成交量待权威数据纠正),不用干等。K线本身每 60 秒(或切代码/切周期时立即)向数据源整体刷新一次做权威纠正。
  周一至周五 9:15(含开盘集合竞价)-11:30、13:00-15:00(北京时间)按设定间隔轮询,非交易时段自动降频到 5 分钟一次,
  标签页切到后台也会暂停请求,长期开着挂机不会一直高频打接口。
  行情与K线均来自新浪财经(主)/腾讯财经(备)的公开接口,浏览器直接请求,不经过本站服务器;两家都请求不到时格子会保留上一次
  数据并提示"获取失败",自动按间隔重试。
