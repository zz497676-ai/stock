...
- 六宫格主视图默认**日K**(更贴合盯盘和爆量预警的默认视角)。现价/涨跌幅每 5 秒(可调 3~60 秒,也可暂停)随实时报价刷新,
  同步推动最新一根K线:今天的日K还没走完就原地更新收盘价/最高/最低/成交量,服务端还没吐出今天这根就先补一根;
  分钟K周期里如果时间已经跨入下一根,也会先补一根新K线(成交量待权威数据纠正),不用干等。K线本身每 60 秒
  (或切代码/切周期时立即)向数据源整体刷新一次做权威纠正。周一至周五 9:15(含开盘集合竞价)-11:30、13:00-15:00
  (北京时间)按设定间隔轮询,非交易时段自动降频到 5 分钟一次,标签页切到后台也会暂停请求,长期开着挂机不会一直高频打接口。
