...
[打开看盘页](https://zz497676-ai.github.io/stock/quotes.html)(纯前端,自选最多 6 只股票/指数同屏看行情):

- 每个格子里直接填代码,自动识别市场前缀(6 开头=沪、0/3 开头=深、4/8 开头=北交所),指数需自带前缀
  (如 `sh000001` 上证指数、`sz399001` 深证成指、`sz399006` 创业板指);代码保存在浏览器本地,下次打开还在。
- 行情由浏览器直接请求新浪/腾讯的公开行情接口,不经过本站服务器;默认每 5 秒自动刷新一次(可调 3~60 秒,
  也可暂停),标签页切到后台会自动停止请求。两家接口都连不上时格子会提示"获取失败"并保留上一次数据,
  按设定间隔自动重试。
